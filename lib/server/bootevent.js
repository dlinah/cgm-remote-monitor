...
function boot (env, language) {
  ...
  function checkNodeVersion (ctx, next) {
    ...
    if (isLTS && (semver.satisfies(nodeVersion, '^22.0.0') || semver.satisfies(nodeVersion, '^20.0.0') || semver.satisfies(nodeVersion, '^18.0.0') || semver.satisfies(nodeVersion, '^16.0.0') || semver.satisfies(nodeVersion, '^14.0.0'))) {
      //Latest Node 22 LTS is recommended and supported.
  ...
  function setupInternals (ctx, next) {
    ...
    ctx.insulinsettings = require('./insulinsettings')(env, ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    ctx.store.ensureIndexes(ctx.insulinsettings( ), ctx.insulinsettings.indexedFields);
