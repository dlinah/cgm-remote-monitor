...
function create (env, ctx) {
  ...
  const calculatorDist = path.join(process.cwd(), 'insulin-buddy-pwa', 'dist');
  console.log('----Serving calculator app from', calculatorDist);
  if (fs.existsSync(calculatorDist)) {
    app.use('/calculator', express.static(calculatorDist, {
      maxAge
    }));
    app.get('/calculator/*', (req, res) => {
      res.sendFile(path.join(calculatorDist, 'index.html'));
    });
  } else {
    console.warn('Calculator app not found at', calculatorDist);
  }
  ...
  if (env.settings.isEnabled('cors')) {
    ...
    app.use(function allowCrossDomain (req, res, next) {
      ...
      res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Content-Length, X-Requested-With, API_SECRET, api_secret, api-secret');
